...
Both the memristor crossbar and neural networks have one thing in common: the weight matrix. While the crossbar computes its dot product using Ohm's and Kirchhoff's Laws, a neural network does it with floating point arithmetic. Keep in mind that in this simulation, write operations are modeled as instantaneous threshold crossings rather than time-integrated voltage pulses, as duration-dependent switching is not necessary for the learning comparison. Just as how numbers get updated and persist in the weight matrix when training a neural network, the same happens in the conductance matrix of a memristor crossbar. That is the root of this experiment!